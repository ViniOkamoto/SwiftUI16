//
//  ChartView.swift
//  DesingCode
//
//  Created by Vinicius Okamoto on 2023-08-23.
//

import Charts
import SwiftUI

struct ChartView: View {
    var body: some View {
        Chart {
            ForEach(data) {
                item in
                LineMark(
                    x: .value("Day", item.day),
                    y: .value("Value", item.value),
                    series: .value("Year", "2023")
                )
                .cornerRadius(10)
                .interpolationMethod(.catmullRom)
                .foregroundStyle(by: .value("Year", "2023"))
                .symbol(by: .value("Year", "2023"))
            }
            ForEach(data2) {
                item in
                LineMark(
                    x: .value("Day", item.day),
                    y: .value("Value", item.value),
                    series: .value("Year", "2022")
                )
                .cornerRadius(10)
                .interpolationMethod(.catmullRom)
                .foregroundStyle(by: .value("Year", "2022"))
                .symbol(by: .value("Year", "2022"))
            }
        }
        .frame(height: 300)
        .padding(20)
    }
}

struct ChartView_Previews: PreviewProvider {
    static var previews: some View {
        ChartView()
    }
}

struct Value: Identifiable {
    var id = UUID()
    var day: String
    var value: Double
}

let data = [
    Value(day: "Jun 1", value: 230),
    Value(day: "Jun 2", value: 130),
    Value(day: "Jun 3", value: 10),
    Value(day: "Jun 4", value: 50),
    Value(day: "Jun 5", value: 280),
]
let data2 = [
    Value(day: "Jun 1", value: 151),
    Value(day: "Jun 2", value: 400),
    Value(day: "Jun 3", value: 70),
    Value(day: "Jun 4", value: 10),
    Value(day: "Jun 5", value: 980),
]
